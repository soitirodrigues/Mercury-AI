...
**Data:** 2026-07-31T03:18:51.571406
...
**Descrição:** main.py crashou em 3 de 3 execuções:
  Run 1: exit=-1, stderr=TIMEOUT após 60s (daemon em loop — esperado)
  Run 2: exit=-1, stderr=TIMEOUT após 60s (daemon em loop — esperado)
  Run 3: exit=-1, stderr=TIMEOUT após 60s (daemon em loop — esperado)
...
| ID | Severidade | Status | Título |
|----|-----------|--------|--------|
| TEST-001 | CRITICAL | ❌ FAIL | Execução de testes excedeu timeout (20min) |

#### TEST-001: Execução de testes excedeu timeout (20min)

- **Severidade:** CRITICAL
- **Status:** FAIL
- **Localização:** 

**Descrição:** A suíte de testes não completou em 20 minutos.
