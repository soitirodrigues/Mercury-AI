    ...
    def analyze(self, df: pd.DataFrame) -> SupportResistanceAnalysis:
        if len(df) < 60:
            return SupportResistanceAnalysis(
                ...
                explanation=["Dados insuficientes para análise de SR."]
            )
        ...
        return SupportResistanceAnalysis(
            support=nearest_s['center'] if nearest_s else 0.0,
            resistance=nearest_r['center'] if nearest_r else 0.0,
            distance_support=round(price - nearest_s['center'], 2) if nearest_s else 0.0,
            distance_resistance=round(nearest_r['center'] - price, 2) if nearest_r else 0.0,
