    ...
    def analyze(self, symbol="GC=F", avg_volume=None, avg_body=None):
        ...
        try:
            ...
            with self.profiler.stage("RegimeAnalysis"):
                regime = self.regime_engine.analyze(market, smart_money, structure, volume)
            ...
            start = DeterministicClock.utcnow()
            with self.profiler.stage("PriceActionAnalysis"):
                price_action = self.price_action.analyze(df)
            self._record_telemetry("PriceActionAnalysis", start, df, price_action)

            # 3. Build Context (1st build — placeholder risk_assessment, RiskAnalysis needs context)
            ...
            # 4. Build Evidences (with real context for adaptive weighting)
            start = DeterministicClock.utcnow()
            ...
            # 6. Rebuild Context with final risk_assessment (immutable contract)
            context = self.context_builder.build(
                market=market,
                trend=trend_evidences,
                price_action=price_action,
            ...
            self._record_telemetry("ContextRebuild", start, df, context)
            ...
            with self.profiler.stage("TradeFilter"):
                trade_filter_result = self.trade_filter.evaluate(context, evidence_bundle)
            self._record_telemetry("TradeFilter", start, context, trade_filter_result.allowed, warnings=len(trade_filter_result.reasons))
            
            start = DeterministicClock.utcnow()
            with self.profiler.stage("DecisionEngine"):
                decision = self.decision_engine.analyze(context, evidence_bundle, trade_filter_result)
