...
sys.path.insert(0, r"C:\Projetos\Mercury-AI")
from mercury_ai.utils.atomic_io import atomic_json_write
atomic_json_write("C:/Projetos/Mercury-AI/test_target.json", {"test": "data"}, indent=2, 
                  signal_checkpoints=True, status_file="C:/Projetos/Mercury-AI/test_status.json", 
                  handshake_mode=True)
print("SUCCESS")
"""
]
...
try:
    ...
    print(f"replace_confirmed value via get: {result.get('replace_confirmed', 'KEY MISSING')}")
    print(f"target_is_NEW value via get: {result.get('target_is_NEW', 'KEY MISSING')}")
    print(f"parent_observation value via get: {result.get('parent_observation', 'KEY MISSING')}")
    ...
    if handshake_valid and result["output_file_valid_json"]:
        ...
        print("Not in first branch - would go to elif or else")
