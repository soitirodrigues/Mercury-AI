...
def run_single_replay(replay_idx, data, symbol):
    ...
    # Get clock state after restore
    clock_now = DeterministicClock.utcnow()
    print(f"Replay #{replay_idx}: DeterministicClock.utcnow() = {clock_now}")
    ...
    # Check contamination
    contaminated = clock_after is not None
    ...
    result = {
        ...
        "clock_contaminated": contaminated,
    }
    
    print(f"=== Replay #{replay_idx} COMPLETE: {len(metrics_list)} metrics ===")
    return result
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=n_parallel) as executor:
        futures = {
            executor.submit(run_single_replay, i, data, symbol): i 
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                print(f"OK Replay {idx} completed successfully")
            except Exception as e:
                print(f"FAIL Replay {idx} failed with error: {e}")
                import traceback
                traceback.print_exc()
    ...
    if all_have_results and all_no_contamination:
        ...
        print("\nS31-03 CLASSIFICATION: RISK OBSERVED")
    ...
    return 0 if (all_have_results and all_no_contamination) else 1
